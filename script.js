/*1.
Напишіть функцію sumSliceArray(arr, first, second), яка приймає масив arr і два числа (first и second) – порядкові номери елементів масиву,
...
function sumSliceArray(arr, first, second) {
  if (typeof first !== "number" || typeof second !== "number") {
    throw new Error("The sequence numbers must be numbers");
  }

  if (first >= arr.length || second >= arr.length) {
    throw new Error("The sequence numbers exceed the length of the array");
  }
...
try {
  ...
  const result = sumSliceArray(myArray, 1, 3);
...
якщо:вік користувача менше 18 або більше 70 років (генерується помилка типу RangeError).користувач не ввів жодних даних в будь-якому полі
(виводиться повідомлення The field is empty! Please enter your age з типом помилки Error). У полі статус введено неправильне слово (тип помилки EvalError).
...
function checkAge() {
  try {
    ...
    if (isNaN(age)) {
      throw new Error("A non-numeric value is entered in the age field");
    }
    ...
    if (!status) {
      ...
      throw new EvalError("Incorrect status word entered");
    }

    if (age < 18 || age > 70) {
      throw new RangeError("Your age does not meet the requirements");
    }

    alert(`Welcome, ${name}! You have access to watch the movie.`);
  } catch (error) {
